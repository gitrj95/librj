...
pair a release with an acquire when we copy the value into the buffer
and publish it for the consumer to see. We can use the above trick to
avoid needing to use an acquire barrier in the consumer in most
cases. Namely, as the consumer's cached copy is behind the true writer
pointer, the memory up to the cached copy must be visible to the
consumer. If we need to reload the cached copy with the atomic writer
...
two cases. The writes to the reader pointer are atomic, so we have,
that, at minimum, that the load from the producer is indeterminately
sequenced with the store from the consumer. If the load from the
producer happens before the store from the consumer, the writer's
cached reader pointer is behind, and we potentially may fail a write:
harmless. If the store from the consumer happens before the load from
the producer, the writer's cached reader pointer will be updated to a
less stale value: the "correct" behavior. However the sequencing, we
have valid behavior.
