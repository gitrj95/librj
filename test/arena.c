...
void initndelete( void )
{
  ...
  arena a;
  expect(
      !arena_init( &a, buf, 123123 ), "happy-path initialization" );
  expect(
      a.hd == buf, "head pointer is the base address of the buffer" );
...
void alloc( void )
{
  ...
  expect( !( (uintptr_t)n % alignof( int ) ),
      "ensure pointer's aligned" );
  expect( (int *)a.hd < n, "head pointer below allocated pointer" );
  expect(
      (int *)a.tl == n, "tail pointer moves with latest allocation" );
  *n = 123;
  int j;
  ...
  expect( !( (uintptr_t)a.hd % pagesz ),
      "head pointer aligned to page boundary" );
